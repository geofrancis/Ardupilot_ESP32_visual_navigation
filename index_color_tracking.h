...
<html>
<head>
   <title>MAVLink Rover - Precision Timing</title>
   <meta charset="utf-8">
   <meta name="viewport" content="width=device-width,initial-scale=1">
   <script async src="https://docs.opencv.org/master/opencv.js" type="text/javascript" onload="initOpenCV()"></script>
   <style>
      body { font-family: sans-serif; background: #121212; color: #e0e0e0; margin: 0; text-align: center; }
      .container { display: flex; flex-wrap: wrap; justify-content: center; padding: 5px; }
      .column { flex: 1; min-width: 320px; padding: 5px; }
      canvas { max-width: 100%; border: 1px solid #444; border-radius: 4px; background: #000; }
      .controls { background: #1e1e1e; padding: 15px; border-radius: 12px; margin-top: 10px; border: 1px solid #333; }
      button { padding: 10px 15px; margin: 5px; border-radius: 6px; border: none; cursor: pointer; font-weight: bold; }
      #filterToggle { background: #0277bd; color: white; width: 100%; margin-bottom: 10px; }
      input[type=range] { width: 100%; margin: 10px 0; }
      .data-box { color: #00e676; font-weight: bold; font-family: monospace; }
      .label-row { display: flex; justify-content: space-between; font-size: 0.8em; color: #888; }
   </style>
</head>
<body>
    <button id="colorDetect" style="margin:10px; padding:10px 20px; background:#2e7d32; color:white; border-radius:6px; border:none;">SYSTEM READY</button>
    ...
    <div class="container">
        ...
        <div class="column">
            ...
            <div class="controls">
                Target: <span class="data-box">X:<span id="XCMdemo">0</span> Y:<span id="YCMdemo">0</span></span>
                <div style="font-size: 0.7em; color: #555; margin-top: 5px;">Status: <span id="streamStatus">Active</span></div>
            </div>
        </div>
    </div>
      ...
      if(maxIdx >= 0){
          ...
          if(tY < deadzoneY) {
              ...
              if(b_autoFilter) {
                  ...
                  let isStillIn = (pctOutX <= exitTolerance && pctOutY <= exitTolerance && tY > boxTopY);
...
setInterval(async () => {
    try {
        let response = await fetch(location.origin + '/?getProbe');
        let data = await response.text();
        let parts = data.split(',');
        if(parts.length === 2 && !sliderX.matches(':active') && !sliderY.matches(':active')) {
            sliderX.value = parts[0]; sliderY.value = parts[1];
        }
    } catch (e) {}
}, 1000);
</script>
